...
Reason: ${reason}`;
  ...
  console.log("\x1b[41m\x1b[37m[🔐 PROTECTION]\x1b[0m BY ZORONOTDEV ACTIVE 🔥\n");
}
...
» Information : t.me/zoronotdec
☇ Inventor    : ZORONOTDEV
☇ Name Script : ZORO INVICTUS
☇ Version     : 4.0
☇ Status      : BOT AKTIF
`));
...
<blockquote><b>⬡═―—⊱ ⎧  ✞︎𓆩𝐙𝐎𝐑𝐎 𝐈𝐍𝐕𝐈𝐂𝐓𝐔𝐒𓆪✞︎ ⎭ ⊰―—═⬡</b></blockquote>
    ...
    if (data.startsWith('addprem_')) {
        ...
        try {
            await ctx.telegram.sendMessage(
                userId,
                `🎉 <b>Selamat!</b>\n` +
                `Anda sekarang pengguna Premium ZORO INVICTUS!\n` +
                `• Durasi: ${duration} hari\n` +
                `• Berakhir: ${expiryDate}`,
                { parse_mode: "HTML" }
            );
